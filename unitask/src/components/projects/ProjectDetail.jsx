import { useState, useEffect } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContextValue';
import { API_URL } from '../../api/constants';
import { 
  Building, 
  Calendar, 
  Users, 
  Clock, 
  ArrowLeft, 
  DollarSign, 
  Send, 
  Loader, 
  AlertTriangle, 
  CheckCircle,
  ExternalLink,
  MessageSquare
} from 'lucide-react';
import BidForm from './BidForm';
import ProjectBids from './ProjectBids';

const ProjectDetail = () => {
  const { projectId } = useParams();
  const navigate = useNavigate();
  const { currentUser } = useAuth();
  const [project, setProject] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [bidFormOpen, setBidFormOpen] = useState(false);
  const [userBid, setUserBid] = useState(null);
  const [tabView, setTabView] = useState('details'); // 'details', 'bids'
  
  const fetchProject = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/projects/${projectId}`);
      
      if (!response.ok) {
        throw new Error('Failed to fetch project details');
      }
      
      const data = await response.json();
      
      if (data.success) {
        setProject(data.project);
      } else {
        throw new Error(data.message || 'Failed to load project details');
      }
    } catch (error) {
      console.error('Error fetching project:', error);
      setError(error.message || 'Error loading project details');
    } finally {
      setLoading(false);
    }
  };
  
  const fetchUserBid = async () => {
    if (!currentUser) return;
    
    try {
      const response = await fetch(`${API_URL}/api/bids/freelancer/${currentUser.id}`);
      
      if (response.ok) {
        const data = await response.json();
        
        if (data.success) {
          const bid = data.bids.find(b => b.project_id === parseInt(projectId));
          if (bid) {
            setUserBid(bid);
          }
        }
      }
    } catch (error) {
      console.error('Error fetching user bid:', error);
    }
  };
  
  useEffect(() => {
    fetchProject();
  }, [projectId]);
  
  useEffect(() => {
    fetchUserBid();
  }, [currentUser, projectId]);
  
  const calculateDaysRemaining = (deadline) => {
    if (!deadline) return null;
    
    const deadlineDate = new Date(deadline);
    const today = new Date();
    const diffTime = deadlineDate - today;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    
    return diffDays;
  };
  
  const formatDate = (dateString) => {
    if (!dateString) return 'No deadline set';
    
    const date = new Date(dateString);
    return date.toLocaleDateString('en-US', { 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    });
  };
  
  const handleBidSuccess = (bid) => {
    setBidFormOpen(false);
    setUserBid(bid);
    fetchProject(); // Refresh the project to update bid count
  };
  
  const handleContactBusiness = async () => {
    if (!currentUser) {
      navigate('/login', { state: { from: `/projects/${projectId}` } });
      return;
    }
    
    try {
      const response = await fetch(`${API_URL}/api/conversations`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          participantIds: [currentUser.id, project.business_id],
          projectInfo: {
            project_id: project.id,
            title: project.title
          }
        })
      });
      
      const data = await response.json();
      
      if (data.success) {
        navigate(`/chat/${data.conversation.id}`);
      } else {
        throw new Error(data.message || 'Failed to create conversation');
      }
    } catch (error) {
      console.error('Error creating conversation:', error);
      alert('Error starting conversation. Please try again.');
    }
  };
  
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center">
        <Loader className="w-10 h-10 text-purple-500 animate-spin mb-4" />
        <p className="text-gray-400">Loading project details...</p>
      </div>
    );
  }
  
  if (error) {
    return (
      <div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center">
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-6 max-w-md w-full flex flex-col items-center">
          <AlertTriangle className="w-12 h-12 text-red-400 mb-4" />
          <h2 className="text-xl font-bold mb-2">Error Loading Project</h2>
          <p className="text-center text-gray-300 mb-6">{error}</p>
          <Link 
            to="/projects" 
            className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg transition-colors flex items-center gap-2"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to Projects
          </Link>
        </div>
      </div>
    );
  }
  
  if (!project) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center">
        <p>Project not found.</p>
      </div>
    );
  }
  
  // Check if current user is the business owner
  const isBusinessOwner = currentUser && currentUser.id === project.business_id;
  
  // Check if project is closed or already awarded
  const isProjectClosed = project.status !== 'open';
  
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        {/* Navigation */}
        <div className="mb-8">
          <Link 
            to="/projects" 
            className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-4"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to Projects
          </Link>
          
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <h1 className="text-2xl md:text-3xl font-bold">{project.title}</h1>
            
            {/* Status Badge */}
            <div className={`px-4 py-1 rounded-full text-sm self-start ${
              project.status === 'open' ? 'bg-green-500/20 text-green-500' :
              project.status === 'awarded' ? 'bg-blue-500/20 text-blue-500' :
              project.status === 'completed' ? 'bg-purple-500/20 text-purple-500' :
              'bg-gray-500/20 text-gray-500'
            }`}>
              {project.status.charAt(0).toUpperCase() + project.status.slice(1)}
            </div>
          </div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Main Content */}
          <div className="md:col-span-2">
            {/* Tabs */}
            <div className="flex border-b border-white/10 mb-6">
              <button
                onClick={() => setTabView('details')}
                className={`px-4 py-2 border-b-2 ${
                  tabView === 'details' 
                    ? 'border-purple-500 text-white' 
                    : 'border-transparent text-gray-400 hover:text-white'
                }`}
              >
                Project Details
              </button>
              <button
                onClick={() => setTabView('bids')}
                className={`px-4 py-2 border-b-2 flex items-center gap-2 ${
                  tabView === 'bids' 
                    ? 'border-purple-500 text-white' 
                    : 'border-transparent text-gray-400 hover:text-white'
                }`}
              >
                Bids
                <span className="bg-white/10 rounded-full px-2 py-0.5 text-xs">
                  {project.bid_count || 0}
                </span>
              </button>
            </div>
            
            {/* Project Details */}
            {tabView === 'details' ? (
              <>
                <div className="bg-gray-900/80 backdrop-blur-sm border border-white/10 rounded-xl p-6 mb-6">
                  <h2 className="text-xl font-semibold mb-4">Description</h2>
                  <div className="prose prose-invert max-w-none">
                    <p className="whitespace-pre-line text-gray-300">{project.description}</p>
                  </div>
                  
                  {project.skills && project.skills.length > 0 && (
                    <div className="mt-6">
                      <h3 className="text-lg font-medium mb-2">Skills Required</h3>
                      <div className="flex flex-wrap gap-2">
                        {project.skills.map(skill => (
                          <span 
                            key={skill}
                            className="bg-purple-900/30 border border-purple-500/30 text-purple-300 rounded-full px-3 py-1 text-sm"
                          >
                            {skill}
                          </span>
                        ))}
                      </div>
                    </div>
                  )}
                </div>
                
                <div className="bg-gray-900/80 backdrop-blur-sm border border-white/10 rounded-xl p-6">
                  <h2 className="text-xl font-semibold mb-4">About the Client</h2>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-gray-800 rounded-full flex items-center justify-center">
                      {project.company_logo ? (
                        <img 
                          src={project.company_logo} 
                          alt={project.company_name} 
                          className="w-full h-full rounded-full object-cover"
                        />
                      ) : (
                        <Building className="w-6 h-6 text-gray-400" />
                      )}
                    </div>
                    <div>
                      <h3 className="font-medium text-lg">{project.company_name}</h3>
                      <div className="flex items-center gap-2">
                        <span className="text-gray-400 text-sm">{project.company_industry || 'Business'}</span>
                        {project.company_verified && (
                          <span className="bg-blue-900/30 text-blue-300 rounded-full px-2 py-0.5 text-xs">
                            Verified
                          </span>
                        )}
                      </div>
                    </div>
                  </div>
                  
                  {!isBusinessOwner && (
                    <button
                      onClick={handleContactBusiness}
                      className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
                    >
                      <MessageSquare className="w-4 h-4" />
                      Contact Client
                    </button>
                  )}
                </div>
              </>
            ) : (
              // Bids View
              <div className="bg-gray-900/80 backdrop-blur-sm border border-white/10 rounded-xl p-6">
                <ProjectBids 
                  projectId={project.id} 
                  isBusinessOwner={isBusinessOwner} 
                  businessId={project.business_id}
                  projectStatus={project.status}
                />
              </div>
            )}
          </div>
          
          {/* Sidebar */}
          <div className="md:col-span-1">
            <div className="sticky top-4 space-y-6">
              {/* Project Information */}
              <div className="bg-gray-900/80 backdrop-blur-sm border border-white/10 rounded-xl p-6">
                <h3 className="text-lg font-semibold mb-3">Project Details</h3>
                
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <DollarSign className="w-5 h-5 text-gray-400" />
                    <div>
                      <p className="text-sm text-gray-400">Budget</p>
                      <p className="font-medium">
                        {project.budget_min && project.budget_max ? 
                          `$${project.budget_min.toFixed(2)} - $${project.budget_max.toFixed(2)}` : 
                          project.budget_min ? 
                            `From $${project.budget_min.toFixed(2)}` : 
                            project.budget_max ? 
                              `Up to $${project.budget_max.toFixed(2)}` : 
                              'Not specified'
                        }
                      </p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-3">
                    <Calendar className="w-5 h-5 text-gray-400" />
                    <div>
                      <p className="text-sm text-gray-400">Deadline</p>
                      <p className="font-medium">
                        {project.deadline ? formatDate(project.deadline) : 'Not specified'}
                      </p>
                    </div>
                  </div>
                  
                  {project.deadline && (
                    <div className="flex items-center gap-3">
                      <Clock className="w-5 h-5 text-gray-400" />
                      <div>
                        <p className="text-sm text-gray-400">Time Remaining</p>
                        <p className="font-medium">
                          {calculateDaysRemaining(project.deadline) <= 0 ? 
                            'Deadline passed' : 
                            `${calculateDaysRemaining(project.deadline)} days left`
                          }
                        </p>
                      </div>
                    </div>
                  )}
                  
                  <div className="flex items-center gap-3">
                    <Users className="w-5 h-5 text-gray-400" />
                    <div>
                      <p className="text-sm text-gray-400">Bids</p>
                      <p className="font-medium">{project.bid_count || 0}</p>
                    </div>
                  </div>
                </div>
              </div>
              
              {/* Bid Actions */}
              {!isBusinessOwner && (
                <div className="bg-gray-900/80 backdrop-blur-sm border border-white/10 rounded-xl p-6">
                  {userBid ? (
                    <>
                      <h3 className="text-lg font-semibold mb-3">Your Bid</h3>
                      <div className="p-4 bg-white/5 rounded-lg mb-4">
                        <div className="flex justify-between mb-2">
                          <span className="text-gray-400">Bid Amount</span>
                          <span className="font-semibold">${userBid.amount}</span>
                        </div>
                        <div className="flex justify-between mb-2">
                          <span className="text-gray-400">Delivery Time</span>
                          <span>{userBid.delivery_time} days</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">Status</span>
                          <span className={`capitalize ${
                            userBid.status === 'pending' ? 'text-yellow-400' :
                            userBid.status === 'accepted' ? 'text-green-400' :
                            'text-red-400'
                          }`}>
                            {userBid.status}
                          </span>
                        </div>
                      </div>
                      <div className="text-sm text-gray-400 mb-4">
                        <p className="mb-2">Your bid has been submitted.</p>
                        <p>You will be notified if the client accepts your proposal.</p>
                      </div>
                      {userBid.status === 'pending' && (
                        <button
                          onClick={() => setBidFormOpen(true)}
                          className="w-full py-2 bg-white/10 hover:bg-white/20 rounded-lg flex items-center justify-center gap-2"
                        >
                          <Edit className="w-4 h-4" />
                          Edit Bid
                        </button>
                      )}
                    </>
                  ) : (
                    <>
                      <h3 className="text-lg font-semibold mb-3">Submit a Proposal</h3>
                      {isProjectClosed ? (
                        <div className="text-center p-4">
                          <p className="text-gray-400 mb-2">This project is no longer accepting bids</p>
                          <span className="bg-gray-700 text-gray-300 px-4 py-2 rounded-lg inline-block">
                            Project Closed
                          </span>
                        </div>
                      ) : (
                        <>
                          <p className="text-gray-400 mb-6">
                            Submit your proposal to work on this project.
                          </p>
                          <button
                            onClick={() => {
                              if (!currentUser) {
                                navigate('/login', { 
                                  state: { from: `/projects/${projectId}` }
                                });
                                return;
                              }
                              setBidFormOpen(true);
                            }}
                            className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
                          >
                            <Send className="w-4 h-4" />
                            Submit Bid
                          </button>
                        </>
                      )}
                    </>
                  )}
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
      
      {/* Bid Form Modal */}
      {bidFormOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <BidForm 
              projectId={project.id} 
              existingBid={userBid}
              onClose={() => setBidFormOpen(false)}
              onSuccess={handleBidSuccess}
              budgetInfo={{
                min: project.budget_min,
                max: project.budget_max
              }}
            />
            
            <div className="mt-4 flex justify-center">
              <button
                onClick={() => setBidFormOpen(false)}
                className="text-gray-400 hover:text-white"
              >
                Cancel
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default ProjectDetail;
